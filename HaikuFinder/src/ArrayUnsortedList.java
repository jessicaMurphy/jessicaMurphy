...
public class ArrayUnsortedList<T> implements ListInterface<T> {
	...
	// Adds element to this list.
	{
		if (numElements == list.length)
			enlarge() ;
		list[numElements]  = (T) element;
		numElements++;
	}
	
	
	
	public void add(ArrayUnsortedList element)
	// Adds element to this list.
	{
		if (numElements == list.length)
			enlarge() ;
	...
	public boolean remove(int spot)
	// Removes an element e from this list such that e. equals(element)
	// and returns true; if no such element exists, returns false.
	{
		find(spot) ;    
...
//	                The list has not been modified since the most recent 
//	                reset
	//
